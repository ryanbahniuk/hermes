/** Written to $TACK_HOME/tack.config.ts by `tack init`. */
export const STARTER_CONFIG = `// Tack configuration.
// Imported at runtime — must \`export default\` a config object.

export default {
  // Locally checked-out repos Tack may use. The \`description\` guides the
  ...
  // so different models can live in different accounts. Manage with \`tack aws …\`
  ...
  // Tip: \`tack model add <name> <version> --model-id <id> --aws-profile <key>\`
  // appends entries here and resolves the inference profile for you —
  // \`tack model discover\` lists ids.
  models: [
    {
      name: "claude-sonnet",
      version: "4.5",
      ...
      // \`tack model remove claude-sonnet 4.5\` then \`tack model add\` to resolve it):
    ...
    // A non-Anthropic Bedrock model runs through the "tack" runtime.
    // Add "pricing" (USD per 1M tokens) so Tack can compute cost — the
    // tack runtime has no SDK-reported cost like the claude runtime does.
    // {
    //   name: "llama",
    //   version: "3.3-70b",
    //   provider: "meta",
  ...
  // Fallback models per role (set via \`tack model set-default <role> <model>\`).
  // Used when nothing higher-priority applies.
  defaults: {
    plannerModel: "claude-sonnet",
    // implementerModel: "claude-sonnet",

    // Cheap/fast model for lightweight chores — e.g. \`tack project add\`
  ...
  // Hard pins per role (set via \`tack model set <role> <model>\`). These win
