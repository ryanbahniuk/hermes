...
/**
 * Builds a LangChain chat model for the `tack` runtime via the Bedrock Converse
...
export function createChatModel(model: ResolvedModel) {
  if (model.target.kind !== "bedrock") {
    throw new Error(
      `the tack runtime requires a bedrock target (model "${model.name}" resolved to "${model.target.kind}")`,
    );
  }
